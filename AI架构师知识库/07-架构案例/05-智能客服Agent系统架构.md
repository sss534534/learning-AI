...
## 元数据
- **难度**: ⭐⭐⭐
- **前置知识**: [Agent系统设计](../04-Agent系统架构/Agent架构模式.md), [RAG系统设计基础](../01-LLM基础理论/RAG.md)
...
| 决策点 | 未选方案 | 未选理由 |
|--------|---------|---------|
| 编排框架 | CrewAI | CrewAI是Agent并行执行，不适合客服的分步流程；LangGraph的状态机模型更匹配 |
| 编排框架 | 自研代码 | 客服流程的状态转换复杂（转人工、情绪降级、多轮上下文），自研容易遗漏边界状态 |
| 编排框架 | Temporal | 太重。Temporal适合异步长周期工作流，客服Agent需要毫秒级响应和热路径 |
| 主力LLM | Claude Sonnet | GPT-4o在中文电商场景的工具调用成功率略高，且当时团队的API预配已基于Azure |
| 主力LLM | Qwen-72B | 自部署成本高，且客服场景的延迟SLO（<3s）比自部署的经济性更重要 |
| 知识检索 | ES全文搜索 | 纯关键词无法处理同义词和语义匹配（如"退货"和"退款"是不同流程但语义相近） |
| 情绪识别 | LLM直接识别 | LLM延迟高、成本高，BERT微调模型20ms即可完成情绪分类，准确率>95% |

**原则：** 客服Agent的核心矛盾是"实时性 vs 质量"。组件的延迟预算必须明确控制（情绪识别<50ms、检索<200ms、LLM<2s），否则整体SLO无法达标。
