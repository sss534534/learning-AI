...
- **关联文件**: [LangGraph与CrewAI实战对比](./02-LangGraph与CrewAI实战对比.md), [OpenAI Agents SDK与MS Agent Framework](./03-OpenAI%20Agents%20SDK与MS%20Agent%20Framework.md), [开源框架Hermes与ClaudeCode](./04-开源框架Hermes与ClaudeCode.md), [Agent脚手架与快速启动工具生态](./05-Agent脚手架与快速启动工具生态.md), [生产级框架选型批判性决策指南](./06-生产级框架选型批判性决策指南.md), [Agent协议与通信架构](../04-Agent系统架构/03-Agent协议与通信架构.md)
- **最后更新**: 2026-06-26
...
框架选型的失败往往不是因为技术缺陷，而是组织的接受能力和基础设施准备不足。

**四维评估模型**

| 维度 | 评估问题 | 低就绪度 (1-2分) | 中就绪度 (3-4分) | 高就绪度 (5分) |
|------|---------|-----------------|-----------------|---------------|
| **团队成熟度** | 团队是否有 Agent 开发经验？ | 无相关经验，需从零搭建 | 有 LLM API 使用经验，无框架经验 | 有框架使用经验，能独立做技术决策 |
| **基础设施** | 部署和运维能力是否到位？ | 无 GPU/容器编排能力 | 有基础 Docker 部署能力 | K8s/Helm/CI-CD 成熟，可支撑生产级部署 |
| **合规安全** | 数据主权和合规要求有多严格？ | 无特殊合规要求 | 需基本数据隔离 | 需完整审计、数据本地化、行业合规 |
| **组织支持** | 管理层对框架迁移的预期是否合理？ | 期待"即插即用"，不分配学习时间 | 接受 1-2 个月学习期 | 分配专项预算，接受 3-6 个月成熟周期 |

**评分与建议**

```
总分 4-8 分（低就绪度）
  → 推荐：OpenAI Agents SDK（托管，免运维）
  → 策略：从最小可行 Agent 开始，先验证价值再扩展
  → 风险：不要同时引入新框架 + 新语言 + 新基础设施

总分 9-14 分（中就绪度）
  → 推荐：LangGraph / CrewAI
  → 策略：选择社区最大的框架，降低学习成本
  → 风险：容易掉入"框架试用陷阱"——每季度换一个，从不深入

总分 15-20 分（高就绪度）
  → 推荐：Hermes Agent / 混合方案
  → 策略：MCP 协议驱动，框架可替换
  → 机会：可以自研内部工具填补框架空白
```
...
选型不是一次性决策，需要持续度量采纳效果：

| 阶段 | 时间窗口 | 关键指标 | 成功标准 |
|------|---------|---------|---------|
| **试点期** | 0-2 月 | PoC 完成度、开发者满意度 NPS | PoC 覆盖 1-2 个核心场景，NPS > 0 |
| **扩展期** | 2-6 月 | 采纳 Agent 数、月活开发者、迭代周期缩短比 | 3+ 团队使用，迭代速度提升 20%+ |
| **生产期** | 6-12 月 | 生产异常率、MTTR、框架迁移 ROI | 异常率 < 1%，MTTR 较基线降 50%，ROI > 2x |
| **平台化期** | 12-18 月 | 内部复用率、新场景接入时间、框架锁定指数 | 复用率 > 60%，新场景接入 < 2 周 |

**框架锁定指数公式**

```
锁定指数 = Σ(每层锁定系数 × 迁移替代难度)
         ───────────────────────────────
                  总评估层数
...
阈值:
  < 0.2: 低锁定，可随时切换
  0.2-0.5: 中锁定，需 1-3 月迁移
  > 0.5: 高锁定，迁移成本可能超过重写
```

> **经验法则**：框架选型中最重要的不是选对框架，而是建立"框架是可替换的"组织共识。
...
- [Agent脚手架与快速启动工具生态](./05-Agent脚手架与快速启动工具生态.md) — AgentStack / AgentX-Kit / Google agents-cli / Mastra / 低代码平台对比
- [生产级框架选型批判性决策指南](./06-生产级框架选型批判性决策指南.md) — The Fool 五模式：暴露假设→钢人论证→死后分析→红队对抗→证据评估
