...
- **关联文件**: [[04-Agent架构评审方法论]], [[06-Agent架构前沿探索]]
- **最后更新**: 2026-06-12
...
Agent 安全防护体系是生产级 Agent 系统不可逾越的底线。本文从输入层、推理层、执行层到输出层构建了四层攻击面全景，深入分析了 Prompt 注入（直接/间接/多模态）、越狱检测、工具调用安全、宪法 AI 护栏、安全沙箱和生产级安全架构六大主题。与传统 Web 安全不同，Agent 系统面临的核心威胁是指令注入和工具滥用——这些在传统应用领域几乎不存在。

三层越狱检测架构（关键词黑名单 + 分类器模型 + LLM 审核）和六层工具调用纵深防御（工具白名单 → 参数验证 → RBAC → 审批门 → 沙箱 → 日志审计）提供了可落地的最佳实践。宪法 AI 将安全规则从"写在文档里"变为"写在代码里"，让 Agent 在每次决策前进行自我审查——这是 Agent 安全区别于传统安全的最重要创新。

安全建设需要与成熟度模型同步推进：P0 阶段至少需要持久化安全配置、集成真实认证、工具权限校验；P1 阶段引入宪法 AI 和审批流；P2 阶段才考虑安全沙箱和形式化验证。安全不是一次性工程，而是伴随系统全生命周期的持续建设。

## Checklist

- [ ] 输入层是否有 Prompt 注入和越狱检测？
- [ ] 是否实现了指令优先级标记的防护？
- [ ] 工具调用是否有参数验证和权限检查？
- [ ] 高风险工具是否有二次确认或审批门？
- [ ] 是否部署了宪法 AI 护栏系统？
...
- [ ] 是否集成真实认证系统而非全内存实现？
- [ ] 安全配置是否持久化不丢失？
- [ ] 是否有安全监控 Dashboard 指标？

## 延伸阅读

- [[04-Agent架构评审方法论]] — 安全维度的评审要点
- [[06-Agent架构前沿探索]] — 形式化验证与安全自修改
- [[02-Multi-Agent协作架构]] — 多 Agent 通信安全
- [[05-Agent成熟度模型与进阶路线图]] — 安全能力建设路径
- [[01-Agent基础架构]] — 单 Agent 安全基础
