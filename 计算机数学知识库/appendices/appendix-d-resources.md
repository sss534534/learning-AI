# 附录D：学习资源推荐

> 整理计算机数学相关的优秀学习资源，包括书籍、视频、课程、在线资源等。

---

## 1. 基础数学

### 1.1 离散数学
- **书籍**：
  - 《离散数学及其应用》 - Kenneth H. Rosen
  - 《具体数学》 - Graham, Knuth, Patashnik
  - 《离散数学》 - 左孝凌等

- **在线资源**：
  - [MIT 6.042J: Mathematics for Computer Science](https://ocw.mit.edu/courses/6-042j-mathematics-for-computer-science-spring-2015/)
  - [可汗学院：离散数学](https://www.khanacademy.org/math)

### 1.2 组合数学
- **书籍**：
  - 《组合数学》 - Richard A. Brualdi
  - 《组合数学导论》 - Joseph A. Gallian
  - 《Introductory Combinatorics》

---

## 2. 算法与数据结构

### 2.1 算法
- **书籍**：
  - 《算法导论》 - Thomas H. Cormen等
  - 《算法》 - Robert Sedgewick
  - 《数据结构与算法分析》 - Mark Allen Weiss

- **在线课程**：
  - [Algorithms, Part I & II - Coursera](https://www.coursera.org/specializations/algorithms)
  - [MIT 6.006: Introduction to Algorithms](https://ocw.mit.edu/courses/6-006-introduction-to-algorithms-fall-2011/)

### 2.2 图论
- **书籍**：
  - 《图论及其应用》 - Bondy and Murty
  - 《图论导引》 - West
  - 《算法图论》 - Goodaire and Parmenter

---

## 3. 线性代数

- **书籍**：
  - 《线性代数及其应用》 - Gilbert Strang
  - 《矩阵分析》 - Horn and Johnson
  - 《Linear Algebra Done Right》 - Sheldon Axler

- **在线课程**：
  - [MIT 18.06: Linear Algebra - Gilbert Strang](https://ocw.mit.edu/courses/18-06-linear-algebra-spring-2010/)
  - [3Blue1Brown线性代数的本质](https://www.3blue1brown.com/topics/linear-algebra)

---

## 4. 微积分与优化

- **书籍**：
  - 《微积分学教程》 - F.M.菲赫金哥尔茨
  - 《Convex Optimization》 - Boyd and Vandenberghe
  - 《凸优化》 - Stephen Boyd

- **在线资源**：
  - [3Blue1Brown微积分的本质](https://www.3blue1brown.com/topics/calculus)
  - [MIT 18.01: Single Variable Calculus](https://ocw.mit.edu/courses/18-01sc-single-variable-calculus-fall-2010/)

---

## 5. 概率与统计

- **书籍**：
  - 《概率论与数理统计教程》 - 茆诗松
  - 《Probability and Statistics》 - Morris H. DeGroot
  - 《统计学习方法》 - 李航

- **在线课程**：
  - [MIT 6.041: Probability Systems Analysis](https://ocw.mit.edu/courses/6-041-probabilistic-systems-analysis-and-applied-probability-fall-2010/)
  - [Khan Academy: Statistics](https://www.khanacademy.org/math/statistics-probability)

---

## 6. AI与深度学习数学

- **书籍**：
  - 《深度学习》 - Goodfellow, Bengio, Courville
  - 《Pattern Recognition and Machine Learning》 - Christopher M. Bishop
  - 《机器学习》 - 周志华

- **在线资源**：
  - [Neural Networks and Deep Learning](http://neuralnetworksanddeeplearning.com/)
  - [Deep Learning Book](https://www.deeplearningbook.org/)

---

## 7. 数论与密码学

- **书籍**：
  - 《初等数论及其应用》 - Kenneth H. Rosen
  - 《数论导引》 - 华罗庚
  - 《密码学原理与实践》 - Douglas R. Stinson

- **在线资源**：
  - [Cryptography I - Coursera](https://www.coursera.org/learn/crypto)

---

## 8. 在线学习平台

- **综合平台**：
  - [Coursera](https://www.coursera.org/)
  - [edX](https://www.edx.org/)
  - [Khan Academy](https://www.khanacademy.org/)
  - [MIT OpenCourseWare](https://ocw.mit.edu/)

- **编程练习**：
  - [LeetCode](https://leetcode.com/)
  - [Project Euler](https://projecteuler.net/)
  - [Codeforces](https://codeforces.com/)

---

## 9. 视觉化学习资源

- **数学可视化**：
  - [3Blue1Brown](https://www.3blue1brown.com/)
  - [Desmos](https://www.desmos.com/calculator)
  - [GeoGebra](https://www.geogebra.org/)

- **算法可视化**：
  - [Visualgo](https://visualgo.net/en)
  - [Algorithm Visualizer](https://algorithm-visualizer.org/)

---

## 10. 学习路径建议

### 10.1 入门路径
1. 离散数学基础（逻辑、集合、证明）
2. 组合数学入门
3. 算法复杂度分析
4. 数据结构基础
5. 图论基础

### 10.2 进阶路径
1. 高级算法设计
2. 计算复杂度理论
3. 线性代数深入
4. 概率与统计
5. 优化理论

### 10.3 专业方向
- **后端/系统开发**：算法、数据结构、图论
- **AI/机器学习**：线性代数、概率统计、优化
- **密码学/安全**：数论、抽象代数、密码学
- **游戏/图形**：线性代数、几何、数值计算

---

## 11. 其他推荐资源

- **网站**：
  - [Math Stack Exchange](https://math.stackexchange.com/)
  - [CS Stack Exchange](https://cs.stackexchange.com/)
  - [OEIS](https://oeis.org/) - 整数序列百科

- **YouTube频道**：
  - 3Blue1Brown
  - MIT OpenCourseWare
  - Computerphile

