...
## 元数据
- **难度**: ⭐⭐⭐
- **前置知识**: `../01-LLM应用开发基础/01-LLM-API调用与集成.md` | `../01-LLM应用开发基础/02-Prompt工程实战指南.md`
- **关联文件**: `../02-RAG应用实战/02-RAG完整实战项目.md` | `../../AI架构师知识库/03-检索增强生成(RAG)原理.md`
- **最后更新**: 2026-06-12
---
...
构建生产级 RAG 应用的关键在于"检索质量决定回答质量的下限"。分块策略是 RAG 系统的第一道关卡：chunk_size 过大会引入噪声降低检索精度，过小则会丢失上下文导致语义不完整。推荐策略是 RecursiveCharacterTextSplitter 配合文档结构感知的分隔符优先级，300-500 字符的块大小适合问答场景，1000-2000 适合摘要场景。元数据（来源、页码、时间戳）的完整标注是后续质量追踪和增量更新的基础。

查询处理是 RAG 与传统搜索的核心区别。查询重写、多查询扩展和 HyDE 三种技术分别解决"表述不精确""视角单一""Query-文档语义鸿沟"三类问题。重排序（Rerank）是从粗排到精排的关键步骤——向量检索快但精度有限，Cross-encoder 精度高但计算代价大，合理的架构是向量检索 Top-K→Rerank 精选 Top-N 的两阶段流水线。在生产环境中，混合检索（向量+BM25）通常比纯向量检索提升 10-20% 的召回率。

## Checklist

- [ ] 根据场景选择合适的分块策略和 chunk_size（问答 300-500，摘要 1000-2000）
- [ ] 为每个 chunk 标注完整的元数据（来源、页码、时间戳、文档类型）
- [ ] 配置 Embedding 模型和向量数据库的索引参数（HNSW/IVF）
- [ ] 实现查询重写 / 多查询扩展 / HyDE 至少一种查询优化
- [ ] 部署重排序环节（Cohere Rerank / BGE-Reranker）
- [ ] 配置混合检索（向量检索 + BM25 关键词检索加权）
- [ ] 设计带引用标注的 RAG Prompt，明确"只基于参考信息"
- [ ] 实现增量索引更新和文档去重机制
- [ ] 部署语义缓存优化重复查询的响应速度
- [ ] 建立检索质量监控指标（召回率、MRR、NDCG）

## 延伸阅读

- `../02-RAG应用实战/02-RAG完整实战项目.md` — 本文实战配套完整项目，涵盖工程化落地的全部模块
- `../01-LLM应用开发基础/02-Prompt工程实战指南.md` — 深入理解 RAG Prompt 的设计原则和优化方法
- `../../AI架构师知识库/03-检索增强生成(RAG)原理.md` — RAG 的理论基础，包括检索模型和融合策略
- `../04-AI应用架构设计/01-AI应用架构设计.md` — RAG 系统在生产架构中的定位和与周边服务的集成
- `../../AI架构师知识库/04-向量数据库选型与优化.md` — 向量数据库的索引原理、性能调优和生产部署
