...
## 元数据
- **难度**: ⭐⭐
- **前置知识**: `../01-LLM应用开发基础/01-LLM-API调用与集成.md`
- **关联文件**: `../02-RAG应用实战/01-RAG应用开发实战.md` | `../../AI架构师知识库/01-大模型基础概念.md`
- **最后更新**: 2026-06-12
---
...
Prompt 工程已从经验性技巧发展为系统化工程方法论。核心转变在于从"写 Prompt"升级为"设计 Prompt 系统"：通过 CLAUDE 框架（清晰、分层、可执行、无歧义、可调试）建立设计规范，通过 A/B 测试框架量化优化效果，通过版本管理追踪 Prompt 变更。Few-Shot 示例的质量比数量更重要，3-5 个高质量、多样性的示例远好于 10+ 个随意示例。

高级 Prompt 技术如 Chain-of-Thought、Tree-of-Thoughts、Directional Stimulus 等，本质上是将模型推理过程结构化的手段。Prompt Chaining 通过任务分解降低单次调用复杂度，适用于长文档处理和复杂推理；ToT 通过多路径探索和评估适用于规划类任务。安全方面，Prompt 注入防护需要构建"输入检测 + 消息隔离 + 输出验证"三道防线，单纯依赖系统 Prompt 强化是不够的。

## Checklist

- [ ] 遵循 CLAUDE 框架设计 Prompt（清晰、分层、可执行、无歧义、可调试）
- [ ] 为关键 Prompt 建立评估基准和 A/B 测试机制
- [ ] 实现 Few-Shot 示例的版本管理和效果追踪
- [ ] 对复杂任务应用 Prompt Chaining 或 ToT 等高级模式
- [ ] 构建 Prompt 注入防护的三道防线（输入检测 / 消息隔离 / 输出验证）
- [ ] 设计角色 Prompt 时明确专业背景、经验年限和风格约束
- [ ] 为每个 Prompt 模板定义输出格式和结构约束
- [ ] 实现结构化输出（JSON Schema / Markdown 表格）的解析与校验
- [ ] 优化 Prompt 参数（温度、top_p、max_tokens）适配不同任务类型
- [ ] 建立 Prompt 变更的评审和回滚机制

## 延伸阅读

- `../01-LLM应用开发基础/01-LLM-API调用与集成.md` — Prompt 的最终效果通过 API 调用实现，需理解参数对生成的影响
- `../02-RAG应用实战/01-RAG应用开发实战.md` — RAG 场景下的 Prompt 设计（带上下文的指令约束）
- `../../AI架构师知识库/01-大模型基础概念.md` — 理解 Transformer 架构和注意力机制有助于更有效地设计 Prompt
- `../03-Agent开发/01-AI-Agent开发实战.md` — Agent 场景下的 System Prompt 设计和工具描述技巧
