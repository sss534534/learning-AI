...
## 元数据
- **难度**: ⭐⭐⭐
- **前置知识**: [ch10-llm-training.md], [ch11-reinforcement-learning.md]
- **关联文件**: [ch14-moe-architecture.md], [ch17-advanced-optimizers.md]
- **最后更新**: 2026-06-12
...
### Test-Time Compute的数学本质

推理模型的核心创新在于将计算资源从训练阶段部分转移到推理阶段。从数学角度看，这等价于将"参数中的知识"部分替换为"计算中的知识"——通过思维链的多步展开，模型在推理时执行了一个小型搜索过程。搜索的深度和广度共同决定了推理质量，二者受幂律关系约束。

### 过程奖励 vs 结果奖励

PRM对推理的每一步给出奖励信号，ORM仅对最终结果评分。在长链推理中，最终答案正确但中间某步错误时，ORM无法定位错误，PRM可以在错误发生的步骤直接惩罚。这使得PRM引导的搜索在数学证明等需要精确推理的任务上显著优于ORM。

---

## 推理模型实践Checklist

- [ ] 理解Test-Time Compute Scaling的幂律关系
- [ ] 掌握并行采样与串行验证的数学差异
- [ ] 理解GRPO算法中组内归一化的数学原理
- [ ] 能够分析推理预算与准确率的量化关系
- [ ] 了解PRM的自适应标注方法
