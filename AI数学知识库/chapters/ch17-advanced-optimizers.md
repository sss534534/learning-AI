...
## 元数据
- **难度**: ⭐⭐⭐
- **前置知识**: [ch01-linear-algebra.md](./ch01-linear-algebra.md), [ch02-calculus.md](./ch02-calculus.md)
- **关联文件**: [ch10-llm-training.md], [ch14-moe-architecture.md]
- **最后更新**: 2026-06-12
...
从SGD到AdamW再到Muon和Sophia，每个新优化器都在收敛速度、泛化性能和计算效率之间寻找更优平衡。SGD泛化最优但收敛慢；Adam用自适应学习率换取快速收敛但泛化不如SGD；AdamW解耦权重衰减修复了Adam的正则化缺陷；Muon引入矩阵正交化原理在大模型训练中展现突破性效果。

### 大模型时代的优化器需求

大模型训练对优化器提出新要求：内存效率（Hessian矩阵不可行）、通信效率（梯度压缩）、数值稳定性（FP8训练需要数值鲁棒性）。Sophia通过Hutchinson估计器近似Hessian对角线，以可接受的计算开销换取了更快的收敛。2026年的趋势是优化器与训练框架的联合设计。
...
- [ ] 理解SGD动量的物理类比和数学形式
- [ ] 掌握Adam/AdamW的矩估计偏差校正原理
- [ ] 理解权重衰减与L2正则化的等价性
- [ ] 了解Muon的矩阵正交化更新机制
- [ ] 掌握学习率调度与优化器的联合调优
- [ ] 理解混合精度训练下优化器的数值稳定性
- [ ] 了解Sophia的Hessian近似方法
- [ ] 掌握分布式训练中优化器的梯度同步模式
- [ ] 理解不同优化器在transformer训练中的表现差异
- [ ] 了解优化器超参数（β1/β2/ϵ）的调优策略

---

## 延伸阅读

- [微积分](ch02-calculus.md)
